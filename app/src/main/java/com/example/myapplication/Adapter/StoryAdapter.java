...
import com.example.myapplication.Utils.AddStoryActivity;
import com.example.myapplication.Utils.StoryActivity;
...
public class StoryAdapter extends RecyclerView.Adapter<StoryAdapter.ViewHolder>{
    ...
    private void userInfo(final ViewHolder viewHolder, String userid, final int pos){
        DatabaseReference reference = FirebaseDatabase.getInstance().
                getReference(mContext.getString(R.string.dbname_user_account_settings)).child(userid);
        reference.addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                UserAccountSettings user = dataSnapshot.getValue(UserAccountSettings.class);

                Glide.with(mContext).load(user.getProfile_photo()).into(viewHolder.story_photo);
                if (pos != 0) {
                    Glide.with(mContext).load(user.getProfile_photo()).into(viewHolder.story_photo_seen);
